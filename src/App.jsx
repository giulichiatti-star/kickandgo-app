  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase.from('profiles')
        .select('entrenador, club_nombre').eq('id', u.user.id).maybeSingle()
      setPerfil(data)
    })()
  }, [])
  const nombre = perfil?.entrenador?.split(' ')[0] || ''
  const club = perfil?.club_nombre || ''
  const saludo = nombre ? `Hola, soy ${nombre}` : 'Hola'
  const contexto = club ? ` (entrenador de ${club})` : ''
  const msg = encodeURIComponent(`${saludo}${contexto}. Tengo una consulta sobre Kick and Go:`)
  return (
    <a href={`https://wa.me/34628584985?text=${msg}`}
